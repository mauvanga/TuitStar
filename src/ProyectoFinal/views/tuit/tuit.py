...
def delete_twit():
    ...
    if(request.referrer.endswith("/tuit/mistuits")):
        ...
        return flask.redirect(flask.url_for('tuit.buscartuits', edBusqueda=palabra_clave))
    else:
        return flask.redirect("/")


@tuit_bp.route('/buscarTuits', methods=['POST', 'GET'])
def buscartuits(edBusqueda=None):
        edBusqueda = flask.request.args.get("edBusqueda") or flask.request.form.get("edBusqueda")
        tuits_filtrados = filtrartuits(edBusqueda)
        return flask.render_template('tuitsbuscados.html', tuits=tuits_filtrados)


def filtrartuits(edBusqueda=None):
    palabra_clave = edBusqueda

    if not palabra_clave:
        tuits_encontrados = list(srp.load_all(TuitDto))
        session['palabra_clave'] = ''
    else:
